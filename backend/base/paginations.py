...
from ninja.pagination import PaginationBase, AsyncPaginationBase
...
class CursorPagination[T: Model](PaginationBase):
    ...
    def _parse_cursor_value(
        self, cursor: str, queryset: QuerySet[T], field_name: str
    ) -> Any:
        """
        ...
        try:
            ...
            if "IntegerField" in field_type or field_type in [
                "AutoField",
                "BigAutoField",
            ]:
                return int(cursor)
    ...
    def paginate_queryset(
        ...
        **params: Any,
        ...
        if not cursor or direction == "after":
            ...
            has_prev = bool(
                cursor
            )  # 커서가 존재했다면 이전 페이지도 존재할 수 있다고 가정

            # 'next' 커서는 리스트의 마지막 항목을 기준으로 설정
            next_cursor = (
                self._get_cursor_value(items_list[-1], field_name)
                if has_next and items_list
                else None
            )
            # 'prev' 커서는 리스트의 첫 번째 항목을 기준으로 설정
            prev_cursor = (
                self._get_cursor_value(items_list[0], field_name)
                if has_prev and items_list
                else None
            )
...
class AsyncCursorPagination[T: Model](AsyncPaginationBase):
    """
    Django Ninja를 위한 양방향 커서 기반 페이지네이션 구현 클래스.
    'before' (이전/구형 데이터) 및 'after' (다음/신규 데이터) 탐색을 지원합니다.
    """
    ...
    def process_after_pagination(
        ...
        **params: Any,
        ...
        if cursor and cursor != "None":
            ...
            # 최초 요청 (커서 없음) 시: 최신 항목부터 보여주기 위해 내림차순 정렬
            queryset = queryset.order_by(f"-{field_name}")
        return cursor, direction, field_name, limit, queryset

    def process_before_pagination(
        self,
        cursor: str,
        direction: str,
        field_name: str,
        limit: int,
        paginated_items: List[T],
    ) -> Any:
    ...
    async def apaginate_queryset(
        ...
        **params: Any,
        ...
        cursor, direction, field_name, limit, queryset = self.process_after_pagination(
            queryset, pagination, request, **params
        )
        # 2. 항목 가져오기 (limit + 1)
        paginated_items = [item async for item in queryset[: limit + 1]]
        return self.process_before_pagination(
            cursor, direction, field_name, limit, paginated_items
        )
