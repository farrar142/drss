...
export const usePagination = <T extends { id: number }>(
  ...
  const [nextCursor, setNextCursor] = useState<string | null>(null);  // for "before" direction (older items)
  const [newestCursor, setNewestCursor] = useState<string | null>(null);  // for "after" direction (newer items)
